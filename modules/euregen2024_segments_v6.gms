...
$if not  set hours  $gdxin precal\precal_%n%.gdx
$if      set hours  $gdxin database\setpar_%nh%.gdx
$load hours,number,load,loadmax,load_sec,loss,vrsc,vrsc_exi
$load irnwflh_h
$load irnwflh_s
$load irnwflh_exi_h
$load irnwflh_exi_s
...
* Set inflow of pump storage to zero as the inflow rate only calculates from total production (and ps should habve no inflow, only reservoir storage)
inflow(s,jpump(j),v,r,t) = 0 ;
inflow_nv(s,jpump(j),r,t) = 0 ;
...
inflowtot_h(jres(j),v,r,t)$(dchrgpen(j,v,r) > 0 and sum(vv, gcapt(j,vv,r,t) * ghours(j,vv,r)) > 0) = round(gen_resv(r,t) * gcapt(j,v,r,t) * ghours(j,v,r) / sum(vv, gcapt(j,vv,r,t) * ghours(j,vv,r)) / dchrgpen(j,v,r) * (1 + 0.5 * number * dischrg(j,v,r)),4) ;
inflowtot_s(jres(j),v,r,t)$(dchrgpen(j,v,r) > 0 and sum(vv, gcapt(j,vv,r,t) * ghours(j,vv,r)) > 0) = round(gen_resv(r,t) * gcapt(j,v,r,t) * ghours(j,v,r) / sum(vv, gcapt(j,vv,r,t) * ghours(j,vv,r)) / dchrgpen(j,v,r) * (1 + 0.5 * number * dischrg(j,v,r)),4) ;
inflowtot_nv_h(jres(j),r,t)$(dchrgpennv(j,r) > 0) = round(gen_resv(r,t) / dchrgpennv(j,r) * (1 + 0.5 * number * dischrgnv(j,r)),4) ;
inflowtot_nv_s(jres(j),r,t)$(dchrgpennv(j,r) > 0) = round(gen_resv(r,t) / dchrgpennv(j,r) * (1 + 0.5 * number * dischrgnv(j,r)),4) ;
inflow(s,jres(j),v,r,t) = round(inflowtot_s(j,v,r,t) * 1e+3 / 8760,4) ;
inflow_nv(s,jres(j),r,t) = round(inflowtot_nv_s(j,r,t) * 1e+3 / 8760,4) ;
...
$if not  set nucmustrun     X(s,i,v,r,t) =l=  XCS(s,i,v,r,t) * (1 + (af(s,i,v,r,t)-1)$af(s,i,v,r,t)) * (1 + (reliability(i,v,r)-1)$reliability(i,v,r)) ;
$if      set nucmustrun     X(s,i,v,r,t) =e=  XCS(s,i,v,r,t) * af(s,i,v,r,t) * (1 + (reliability(i,v,r)-1)$reliability(i,v,r)) ;
...
$if not  set biomustrun     X(s,i,v,r,t) =l=  XCS(s,i,v,r,t) * (1 + (af(s,i,v,r,t)-1)$af(s,i,v,r,t)) * (1 + (reliability(i,v,r)-1)$reliability(i,v,r)) ;
$if      set biomustrun     X(s,i,v,r,t) =e=  XCS(s,i,v,r,t) * af(s,i,v,r,t) * (1 + (reliability(i,v,r)-1)$reliability(i,v,r)) ;
...
$if not  set hours  hhours(s,"Storage_ST") = 12 ;
$if not  set hours  hhours(s,"PumpStorage") = 12 ;
$if      set hours  hhours(s,j) = 1 ;

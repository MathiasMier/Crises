...
$if not  set co2mips    $if not  set longrun    round(msr_int("2021"), 4) ;
$if not  set co2mips    $if      set longrun    round(msr_int("2022"), 4) ;
tnacstart_in =
$if      set co2mips     round(tnac_int("2021"), 4) ;
$if not  set co2mips    $if not  set longrun    round(tnac_int("2021"), 4) ;
$if not  set co2mips    $if      set longrun    round(tnac_int("2022"), 4) ;
co2eleuk_in(t) = round(co2eleuk_int(t), 4) ;

loop(t$sameas(t,"2030"),
co2ele_in(t) = round(sum(tmerge(tt), co2ele_int(tt))/nyrs(t), 4) ;
co2ind_in(t) = round(sum(tmerge(tt), co2ind_int(tt))/nyrs(t), 4) ;
co2avi_in(t) = round(sum(tmerge(tt), co2avi_int(tt))/nyrs(t), 4) ;
co2shi_in(t) = round(sum(tmerge(tt), co2shi_int(tt))/nyrs(t), 4) ;
co2out_in(t) = round(sum(tmerge(tt), co2out_int(tt))/nyrs(t), 4) ;
co2indfix_in(t) = round(sum(tmerge(tt), co2indfix_int(tt))/nyrs(t), 4) ;
co2indorgfix_in(t) = round(sum(tmerge(tt), co2indorgfix_int(tt))/nyrs(t), 4) ;
co2elecind(t) = round(sum(tmerge(tt), co2indshare_int(tt))/nyrs(t), 4) ;
co2ele_org(t) = round(sum(tmerge(tt), co2ele_org_int(tt))/nyrs(t), 4) ;
co2ind_org(t) = round(sum(tmerge(tt), co2ind_org_int(tt))/nyrs(t), 4) ;
co2can_in(t) = round(sum(tmerge(tt), co2can_int(tt))/nyrs(t), 4) ;
co2add_in(t) = round(sum(tmerge(tt), co2add_int(tt))/nyrs(t), 4) ;
co2allocated_in(t) = round(sum(tmerge(tt), co2allocated_int(tt))/nyrs(t), 4) ;
co2auctioned_in(t) = round(sum(tmerge(tt), co2auctioned_int(tt))/nyrs(t), 4) ;
tnacuse_in(t) = 
$if not  set hedgeeua    round(sum(tmerge(tt), tnacuse_int(tt))/nyrs(t), 4) ;
$if      set hedgeeua    round(sum(tmerge(tt), tnacresuse_int(tt))/nyrs(t), 4) ;
msrin_in(t) = round(sum(tmerge(tt), msrin_int(tt))/nyrs(t), 4) ;
co2eleuk_in(t) = round(sum(tmerge(tt), co2eleuk_int(tt))/nyrs(t), 4) ;
) ;
...
$if      set longrun     TNAC.FX("2022") = tnac_in("2022") ;
*$if      set longrun     TNAC.UP("2030") = 833 ;
...
$if      set longrun     TNACUSE.FX("2022") = tnacuse_in("2022") ;

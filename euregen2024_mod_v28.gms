...
* The hourly model is always static and does not use a precal routine as this one is too demanding. However, sets are from a specific precal. Parameters in turn come from a hourly database. The set reductions helps reading the model.
...
$if      set static     t        Model time periods /%year%/
...
$gdxin precal\precal_%n%.gdx
$if not  set static    $load t
$load v,oldv, newv, tv, r
$load rwindoff, rnonuc, rnuc, rnolig, rlig, rnocoa, rcoa, rfr, rge, rbr, reu
$load toptimize, tmerge, tmergesub, t2030, topt2023, topt2024_2030, topt2024, topt2030, topt2030plus, t2023, topt2035plus, tcri, told
$gdxIn
...
$gdxin precal\precal_%n%.gdx
$load invlimUP, invlimLO, invlimUP_eu, invlife, invdepr, capcost, fomcost, vomcost, effrate, co2captured, emit, reliability, capcred, mindisp
$load sclim, sclim_eu, deccost
$gdxin
...
$gdxin precal\precal_%n%.gdx
$load ghours, chrgpen, dchrgpen, dischrg, gcapcost, gfomcost, gvomcost, greliability, gcapcred, ginvlife, ginvdepr, ginvlimLO, ginvlimUP, ginvlimUP_eu
$load gcapcostc,gcapcostd,gcapcostr,gfomcostc,gfomcostd,gfomcostr
$load dischrgnv,ghoursnv,chrgpennv,dchrgpennv
$gdxin
...
$gdxin precal\precal_%n%.gdx
$load nyrs, drate, dfact, annuity_ir, annuity_jr, annuity_kr
$gdxin
...
$gdxin precal\precal_%n%.gdx
$load deprtime, glifetime, gdeprtime
...
$gdxin precal\precal_%n%.gdx
$load pfuel, pfadd, pco2_r, pco2, ccscost, ccscost_eu, pfadd_rel
$gdxin
...
* * * biomass_out
parameter
biolim_r(r,t) Upper bounds by region on biomass use (TWh)
biolim(t) Upper bounds by region on biomass use (TWh)
;

$gdxin precal\precal_%n%.gdx
$load biolim_r, biolim
$gdxin
...
discost_int(i,v,r,t)     Dispatch cost (EUR per MWh el) interim paramter for later overwriting
voll(r,t)                Value of lost load (EUR per MWh el)
;

$gdxin precal\precal_%n%.gdx
$load discost, discost_int=discost, voll
$gdxin

*$if      set static voll(r,t) = 1000 ;

$if      set nochp  discost(chp(i),v,r,t) = discost_int(i,v,r,t) + 60 ;
*why +60 ♠ This is the heating premium that is placed on chp capacities beforehand and substracted afterwards

$include    modules\euregen2024_callmin_v1.gms
...
GCR(j,v,r,t)            Energy storage reservoir capacity (GWh)
GCNV(j,r,t)             Energy storage charge-discharge capacity (GW) ? investment? new vintage?
...
* Potential variables
BCR(r,t)                Annual flow of biomass used per region (TWh)
...
$if      set learning   $gdxin precal\precal_%n%.gdx
$if      set learning   $load ivt, ivttv, inotir_lea, notkir_lea
$if      set learning   $gdxin
...
$if not  set hours  $gdxin precal\precal_%n%.gdx
$if      set hours  $gdxin database\setpar_%nh%.gdx
$load ivrttv, jvrttv, tvrttv, ivrt_nottv, jvrt_nottv, v2022_2023
$gdxin
...
retire(ivrt(i,v,r,toptimize(t)))$(not sameas(t,"2050") and not sameas(t,"2022"))..
...
gretire(
$if not  set storagebalnv   jvrt(j,v,r,t))$(not sameas(t,"2050") and not sameas(t,"2022"))..
$if      set storagebalnv   jvrt(nonvj(j),v,r,t))$(not sameas(t,"2050") and not sameas(t,"2022"))..
        GC(j,v,r,t+1) =l= GC(j,v,r,t) ;

gretirec(jvrt(ghyd(j),v,r,t))$(not sameas(t,"2050") and not sameas(t,"2022"))..
        GCC(j,v,r,t+1) =l= GCC(j,v,r,t) ;
gretired(jvrt(ghyd(j),v,r,t))$(not sameas(t,"2050") and not sameas(t,"2022"))..
        GCD(j,v,r,t+1) =l= GCD(j,v,r,t) ;
gretirer(jvrt(ghyd(j),v,r,t))$(not sameas(t,"2050") and not sameas(t,"2022"))..
...
gretirenv(nvj(j),r,toptimize(t))$(not sameas(t,"2050") and not sameas(t,"2022"))..
...
* * * Biomass market
* Biomass flow
bioflow(toptimize(t))..           BC(t)     =e= sum(ivrt(bio(i),v,r,t), round(1 / effrate(i,v,r), 4) * XTWH(i,v,r,t)) ;
bioflow_r(r,toptimize(t))..       BCR(r,t)  =e= sum(ivrt(bio(i),v,r,t), round(1 / effrate(i,v,r), 4) * XTWH(i,v,r,t)) ;
* Biomass use limits
cumbio(toptimize(t))..            BC(t)     =l= biolim(t) ;
cumbio_r(r,toptimize(t))..        BCR(r,t)  =l= biolim_r(r,t) ; 
...
$if      set hydrogensimple     $include    modules\euregen2024_hydrogensimple_v1.gms
$if not  set days               $include    modules\euregen2024_segments_v6.gms
$if      set days               $include    modules\euregen2024_days_v7.gms
...
$if not  set days   sum(rr,  sum((s,tmap(k,rr,r)), hours(s) * E(s,k,rr,r,t))) * 1e-3 =l= imp(r,t) * 1.25 ;
$if      set days   sum(rr,  sum(sd, days(sd) * sum(tmap(k,rr,r), sum(hd, E_D(sd,hd,k,rr,r,t))))) * 1e-3 =l= imp(r,t) * 1.25 ;

expUP(r,topt2023(t))$(expo(r,t) > 0)..
$if not  set days   sum(rr, sum((s,k)$tmap(k,rr,r), hours(s) * E(s,k,r,rr,t))) * 1e-3 =l= expo(r,t) * 1.25 ;
$if      set days   sum(rr, sum(sd, days(sd) * sum(k$tmap(k,rr,r), sum(hd, E_D(sd,hd,k,r,rr,t))))) * 1e-3 =l= expo(r,t) * 1.25 ;

impLO(r,topt2023(t))$(imp(r,t) > 0)..
$if not  set days   sum(rr,  sum((s,k)$tmap(k,rr,r), hours(s) * E(s,k,rr,r,t))) * 1e-3 =g= imp(r,t) * 0.75 ;
$if      set days   sum(rr,  sum(sd, days(sd) * sum(k$tmap(k,rr,r), sum(hd, E_D(sd,hd,k,rr,r,t))))) * 1e-3 =g= imp(r,t) * 0.75 ;

expLO(r,topt2023(t))$(expo(r,t) > 0)..
$if not  set days   sum(rr, sum((s,k)$tmap(k,rr,r), hours(s) * E(s,k,r,rr,t))) * 1e-3 =g= expo(r,t) * 0.75 ;
$if      set days   sum(rr, sum(sd, days(sd) * sum(k$tmap(k,rr,r), sum(hd, E_D(sd,hd,k,r,rr,t))))) * 1e-3 =g= expo(r,t) * 0.75 ;
...
genLO_nucl(r,topt2023(t))$(gen_min2_nucl(r,t) > 0)..
    sum(ivrt(nuc(i),oldv(v),r,t), XTWH(i,v,r,t)) =g= gen_min2_nucl(r,t) * 0.98 ;
...
$if not  set days   $if not  set storagebalnv        sum(jvrt(jpump(j),v,r,t), sum(s, hours(s) * GD(s,j,v,r,t) * dchrgpen(j,v,r))) * 1e-3 =l= gen_pump(r,t) * 1.1 ;
$if      set days   $if not  set storagebalnv        sum(jvrt(jpump(j),v,r,t), sum(sd, days(sd) * sum(hd, GD_D(sd,hd,j,v,r,t) * dchrgpen(j,v,r)))) * 1e-3 =l= gen_pump(r,t) * 1.1 ;
$if not  set days   $if      set storagebalnv        sum(jpump(j), sum(s, hours(s) * GDNV(s,j,r,t) * dchrgpennv(j,r))) * 1e-3 =l= gen_pump(r,t) * 1.1 ;
$if      set days   $if      set storagebalnv        sum(jpump(j), sum(sd, days(sd) * sum(hd, GDNV_D(sd,hd,j,r,t) * dchrgpennv(j,r)))) * 1e-3 =l= gen_pump(r,t) * 1.1 ;
genLO_pump(r,topt2023(t))$(gen_pump(r,t) > 0)..
$if not  set days   $if not  set storagebalnv        sum(jvrt(jpump(j),v,r,t), sum(s, hours(s) * GD(s,j,v,r,t) * dchrgpen(j,v,r))) * 1e-3 =g= gen_pump(r,t) * 0.9 ;
$if      set days   $if not  set storagebalnv        sum(jvrt(jpump(j),v,r,t), sum(sd, days(sd) * sum(hd, GD_D(sd,hd,j,v,r,t) * dchrgpen(j,v,r)))) * 1e-3 =g= gen_pump(r,t) * 0.9 ;
$if not  set days   $if      set storagebalnv        sum(jpump(j), sum(s, hours(s) * GDNV(s,j,r,t) * dchrgpennv(j,r))) * 1e-3 =g= gen_pump(r,t) * 0.9 ;
$if      set days   $if      set storagebalnv        sum(jpump(j), sum(sd, days(sd) * sum(hd, GDNV_D(sd,hd,j,r,t) * dchrgpennv(j,r)))) * 1e-3 =g= gen_pump(r,t) * 0.9 ;
...
$if not  set days   $if not  set storagebalnv        sum(jvrt(jres(j),v,r,t), sum(s, hours(s) * GD(s,j,v,r,t) * dchrgpen(j,v,r))) * 1e-3 =g= gen_resv(r,t) * 0.9 ;
$if      set days   $if not  set storagebalnv        sum(jvrt(jres(j),v,r,t), sum(sd, days(sd) * sum(hd, GD_D(sd,hd,j,r,t) * dchrgpen(j,v,r)))) * 1e-3 =g= gen_resv(r,t) * 0.9 ;
$if not  set days   $if      set storagebalnv        sum(jres(j), sum(s, hours(s) * GDNV(s,j,r,t) * dchrgpennv(j,r))) * 1e-3 =g= gen_resv(r,t) * 0.9 ;
$if      set days   $if      set storagebalnv        sum(jres(j), sum(sd, days(sd) * sum(hd, GDNV_D(sd,hd,j,r,t) * dchrgpennv(j,r)))) * 1e-3 =g= gen_resv(r,t) * 0.9 ;
   
genUP_pump_all(topt2023(t))$(sum(r, gen_pump(r,t)) > 0)..
$if not  set days   $if not  set storagebalnv        sum(jvrt(jpump(j),v,r,t), sum(s, hours(s) * GD(s,j,v,r,t) * dchrgpen(j,v,r))) * 1e-3 =l= sum(r, gen_pump(r,t)) * 1.1 ;
$if      set days   $if not  set storagebalnv        sum(jvrt(jpump(j),v,r,t), sum(sd, days(sd) * sum(hd, GD_D(sd,hd,j,v,r,t) * dchrgpen(j,v,r)))) * 1e-3 =l= sum(r, gen_pump(r,t)) * 1.1 ;
$if not  set days   $if      set storagebalnv        sum(r, sum(jpump(j), sum(s, hours(s) * GDNV(s,j,r,t) * dchrgpennv(j,r)))) * 1e-3 =l= sum(r, gen_pump(r,t)) * 1.1 ;
$if      set days   $if      set storagebalnv        sum(r, sum(jpump(j), sum(sd, days(sd) * sum(hd, GDNV_D(sd,hd,j,r,t) * dchrgpennv(j,r))))) * 1e-3 =l= sum(r, gen_pump(r,t)) * 1.1 ;
genLO_pump_all(topt2023(t))$(sum(r, gen_pump(r,t)) > 0)..
$if not  set days   $if not  set storagebalnv        sum(jvrt(jpump(j),v,r,t), sum(s, hours(s) * GD(s,j,v,r,t) * dchrgpen(j,v,r))) * 1e-3 =g= sum(r, gen_pump(r,t)) * 0.9 ;
$if      set days   $if not  set storagebalnv        sum(jvrt(jpump(j),v,r,t), sum(sd, days(sd) * sum(hd, GD_D(sd,hd,j,v,r,t) * dchrgpen(j,v,r)))) * 1e-3 =g= sum(r, gen_pump(r,t)) * 0.9 ;
$if not  set days   $if      set storagebalnv        sum(r, sum(jpump(j), sum(s, hours(s) * GDNV(s,j,r,t) * dchrgpennv(j,r)))) * 1e-3 =g= sum(r, gen_pump(r,t)) * 0.9 ;
$if      set days   $if      set storagebalnv        sum(r, sum(jpump(j), sum(sd, days(sd) * sum(hd, GDNV_D(sd,hd,j,r,t) * dchrgpennv(j,r))))) * 1e-3 =g= sum(r, gen_pump(r,t)) * 0.9 ;
...
$if not  set static $if      set co2mark    $include    modules\euregen2024_co2cons_v2.gms
$if not  set static $if      set co2iter    $include    modules\euregen2024_co2cons_v2.gms
$if not  set static $if      set co2mips    $include    modules\euregen2024_co2cons_v2.gms

$if not  set static $if      set co2mark    $include    modules\euregen2024_co2mark_v1.gms
$if not  set static $if      set co2iter    $include    modules\euregen2024_co2iter_v1.gms
$if not  set static $if      set co2mips    $include    modules\euregen2024_co2mips_v1.gms
$if not  set static $if      set co2price   $include    modules\euregen2024_co2price_v1.gms

$if not  set static $if      set uselimits  $include    modules\euregen2024_uselimits_v2.gms
$if not  set static $if      set invlimits  $include    modules\euregen2024_invlimit_v1.gms

$if      set static $if  not set hours      $include    modules\euregen2024_static_v2.gms
$if      set static $if      set hours      $include    modules\euregen2024_hours_static_v2.gms

$if      set invtypes                       $include    modules\euregen2024_invtypes_v1.gms
$if      set invendo                        $include    modules\euregen2024_invendo_v1.gms
$if      set biomarket                      $include    modules\euregen2024_biomass_v4.gms
$if      set biomarket_r                    $include    modules\euregen2024_biomass_v4.gms

$include modules\euregen2024_objective_v2.gms    


* * * Model fixes
$if      set longrun     IX.FX(i,r,tmergesub(t)) = 0 ;
$if      set longrun     IG.FX(j,r,tmergesub(t)) = 0 ;
$if      set longrun     IT.FX(k,r,rr,tmergesub(t)) = 0 ;
...
$if not  set static      IX.FX(nuc(i),r,topt2023(t))$(not sum(tv(t,v), capt(i,v,r,t)) > 0) = 0 ;
$if not  set static      IX.FX(nuc(i),r,topt2023(t))$(sum(tv(t,v), capt(i,v,r,t)) > 0) = sum(tv(t,v), capt(i,v,r,t)) ;
...
* Different objectives depending on mixed, not mixed, investor types, or learning
$if      set mixed  $if not  set invtypes           investcost_mixed
$if      set mixed  $if      set invtypes           investcost_mixed_inv
$if      set mixed  $if      set learning           ginvestcost_mixed
$if      set mixed  $if      set learning           tinvestcost_mixed
$if      set mixed  $if      set learning           investcost_mixed_learning
$if not  set mixed  $if not  set invendo            investcost_old
$if not  set mixed  $if      set learning           ginvestcost_old
$if not  set mixed  $if      set learning           tinvestcost_old
$if not  set mixed  $if      set learning           investcost_old_learning
* Investor types translation
$if      set invtypes                               ix_investinv
$if      set invtypes                               ig_investinv
$if      set invtypes                               igc_investinv
$if      set invtypes                               igd_investinv
$if      set invtypes                               igr_investinv
$if      set invtypes                               it_investinv
$if      set invtypes                               investbudget

$if      set invendo                                con_eq0
$if      set invendo $if not set nocredit           con_dbmax
$if      set invendo                                con_eq
$if      set invendo                                con_db
$if      set invendo                                con_re
$if      set invendo                                con_rp
$if      set invendo                                con_investi
$if      set invendo                                con_investj
$if      set invendo                                investcost_endo
...
$if      set static $if not  set storagebalnv    gexlife_static
$if      set static $if      set hydrogensimple  gcexlife_static
$if      set static $if      set hydrogensimple  gdexlife_static
$if      set static $if      set hydrogensimple  grexlife_static
$if      set static $if      set storagebalnv    gexlifenv_static
$if      set static                              tinvestexi_static
$if      set static    $if not  set co2price     euets_static
$if      set static    $if not  set co2price     ukets_static
...
* Naturgal gas, and CCS markets (or limits)
...
$if      set biomarket                           biomarket
$if      set biomarket_r                         biomarket_r
$if      set biotrade                            biolimit
$if      set biomarket2                          sumdbcla
$if      set biomarket2_r                        sumdbclar
$if      set biomarket3                          sumdbcla
$if      set biomarket3_r                        sumdbclar
$if      set biomarket3                          biomarket3lo
$if      set biomarket3                          biomarket3up
$if      set biomarket3_r                        biomarket3lo_r
$if      set biomarket3_r                        biomarket3up_r 
...
$if not  set static $if      set co2marktotal                        co2flow
$if not  set static $if      set co2marktotal                        co2market
$if not  set static $if      set euetsbreak                          co2flow_r
$if not  set static $if      set euetsbreak                          co2market_r
$if not  set static $if      set euetsbreak                          tnac_r
$if not  set static $if      set co2mark                             euco2flow
$if not  set static $if      set co2mark                             euets
$if not  set static $if      set co2mark   $if      set banking      eutnac
$if not  set static $if      set co2mark                             ukco2flow
$if not  set static $if      set co2mark                             ukets
$if not  set static $if      set co2mark   $if      set banking      uktnac
* CO2 market with MSR dynamics via iterative modeling (only shortrun)
$if not  set static $if      set co2iter                             euco2flow
$if not  set static $if      set co2iter                             it_euets
$if not  set static $if      set co2iter   $if      set banking      it_tnac
$if not  set static $if      set co2iter                             ukco2flow
$if not  set static $if      set co2iter                	         ukets
$if not  set static $if      set co2iter   $if      set banking      uktnac
* CO2 market with MSR dynamics as simple as possible (only shortrun)
$if not  set static $if      set co2mips                                euco2flow
$if not  set static $if      set co2mips                                eqs_euets
$if not  set static $if      set co2mips                                eqs_msr
$if not  set static $if      set co2mips                                eqs_msrin2023
$if not  set static $if      set co2mips                                eqs_msrout2023
$if not  set static $if      set co2mips                                eqs_msrout_max
$if not  set static $if      set co2mips                                eqs_msrout_fin
$if not  set static $if      set co2mips                                eqs_msrout_bin
$if not  set static $if      set co2mips                                eqs_msroutup
$if not  set static $if      set co2mips                                eqs_msroutlo
$if not  set static $if      set co2mips    $if      set msrinout       eqs_msrinout
$if not  set static $if      set co2mips                                eqs_tnac
$if not  set static $if      set co2mips                                eqs_tnac_bin
$if not  set static $if      set co2mips                                eqs_cancel2023
$if not  set static $if      set co2mips                                eqs_cancel2045
$if not  set static $if      set co2mips                                eqs_cancel_fin
$if not  set static $if      set co2mips                                eqs_cancel_bin

$if not  set static $if      set co2mips    $if      set euetsold       eqs_msrin_old
$if not  set static $if      set co2mips    $if      set euetsold       eqs_msrout_old
$if not  set static $if      set co2mips    $if      set euetsold       eqs_cancel_old
$if not  set static $if      set co2mips    $if      set euetsold       eqs_cancelup_old             
$if not  set static $if      set co2mips    $if      set euetsold       eqs_cancello_old
$if not  set static $if      set co2mips    $if      set euetsold       eqs_tnacup_old
$if not  set static $if      set co2mips    $if      set euetsold       eqs_tnaclo_old

$if not  set static $if      set co2mips    $if      set euetsmsrin     eqs_msrin_new
$if not  set static $if      set co2mips    $if      set euetsmsrin     eqs_msrout_new
$if not  set static $if      set co2mips    $if      set euetsmsrin     eqs_cancel_old
$if not  set static $if      set co2mips    $if      set euetsmsrin     eqs_cancelup_old             
$if not  set static $if      set co2mips    $if      set euetsmsrin     eqs_cancello_old
$if not  set static $if      set co2mips    $if      set euetsmsrin     eqs_tnacup_old             
$if not  set static $if      set co2mips    $if      set euetsmsrin     eqs_tnaclo_old

$if not  set static $if      set co2mips    $if      set euetscancel    eqs_msrin_old
$if not  set static $if      set co2mips    $if      set euetscancel    eqs_msrout_old
$if not  set static $if      set co2mips    $if      set euetscancel    eqs_cancel_new
$if not  set static $if      set co2mips    $if      set euetscancel    eqs_cancelup_new             
$if not  set static $if      set co2mips    $if      set euetscancel    eqs_cancello_new
$if not  set static $if      set co2mips    $if      set euetscancel    eqs_tnacup_new             
$if not  set static $if      set co2mips    $if      set euetscancel    eqs_tnaclo_new

$if not  set static $if      set co2mips    $if      set euetsnew       eqs_msrin_new
$if not  set static $if      set co2mips    $if      set euetsnew       eqs_msrout_new
$if not  set static $if      set co2mips    $if      set euetsnew       eqs_cancel_new
$if not  set static $if      set co2mips    $if      set euetsnew       eqs_cancelup_new             
$if not  set static $if      set co2mips    $if      set euetsnew       eqs_cancello_new
$if not  set static $if      set co2mips    $if      set euetsnew       eqs_tnacup_new             
$if not  set static $if      set co2mips    $if      set euetsnew       eqs_tnaclo_new

$if not  set static $if      set co2mips                                ukco2flow
$if not  set static $if      set co2mips                                ukets
$if not  set static $if      set co2mips    $if      set banking        uktnac

* Calibration of CO2 markets (2022 and steering)
$if not  set static $if      set real2022                               eqs_co2real2022
$if not  set static $if      set co2mono                                eqs_co2monoUP
* Policy
...
$if      set calibration_nochp      $if not  set biomustrun                         genUP_biom_nochp
...
$if      set calibration_nochp      $if not  set nucmustrun                         genUP_nucl
$if not  set calibration_nochp      $if      set nolimits                           genUP_nucl
*$if      set calibration_nochp      $if not  set nucmustrun     $if      set static genLO_nucl
...
* Transmission
$if      set calibration_trans      $if      set trans                              impUP
$if      set calibration_trans      $if      set trans                              impLO
$if      set calibration_trans      $if      set trans                              expUP
$if      set calibration_trans      $if      set trans                              expLO
$if      set calibration_trans      $if      set trans                              expUP_all
$if      set calibration_trans      $if      set trans                              expLO_all
...
$if      set qcp                          solve euregen using qcp minimizing SURPLUS ;
$if not  set mip $if not  set miqcp $if not  set co2mips $if not  set techmin $if not  set elastic  $if not  set qcp     solve euregen using lp minimizing SURPLUS ;

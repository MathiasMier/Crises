...
$if not  set elastic Electricity1_rpt(t,r,"price-avg-summer")$(sum(s$ssea(s,"s"), load(s,r,t) * hours(s)) > 0)         = sum(s$ssea(s,"s"), price(s,r,t) * load(s,r,t) * hours(s)) / sum(s$ssea(s,"s"), load(s,r,t) * hours(s)) + eps ;
...
ginstalledc(r,j,t)       = sum(jvrt(j,v,r,t), GCL(j,v,r,t)) ;
...
storagerpt /Pump-GenCap,Res-GenCap,Hyd-GenCap,Bat-GenCap,
            ...
            GenCap,ResCap,FromMarket,ToMarket,Hyd-ChaCap,Hyd-Import/
;
...
Storage_rpt(t,r,"Hyd-GenCap")           = sum(j, Storage_ChargeCapacity_rpt(t,r,j)) ;
$if      set hydrogenimport     Storage_rpt(t,r,"Hyd-Import")           = sum(ghyd(j), HIMP.L(j,r,t)) * 1e-3 ;
...
realeffect_rpt("2022","EUR","CO2") = eps ;
